...
void BST::ReverseInNodes(TNode* curNode) {    
  if (curNode!=NULL) {
    ReverseInNodes(curNode->right);
    TravQueue->Enqueue(curNode->item);
    ReverseInNodes(curNode->left);
  }
}
...
// the desired order.
  ...
  switch (order) {
    ...
    case POST_ORDER: PostNodes(root);
                     break;
    case REV_IN_ORDER : ReverseInNodes(root);
    					break;
  }
  
}

int BST::CountNodesMax(TNode* curnode) { //Return total number of nodes in (sub-)tree denoted by curnode
	if (curnode == NULL){
		return -1;
	}
	
	else{
		int left = CountNodesMax(curnode->left) + 1;
		int right = CountNodesMax(curnode->right) + 1;
//		cout<<'left: '<<left<<endl;
