...
double AUList::getMean() {  //Gets arithmetic mean.
    double mean = 0;
    for (int loc = 0; loc < length; loc++) {
        mean += ListItems[loc];
    }
...
double AUList::getStdev() { //Gets the Population Standard Deviation for the list.
    double stdev = 0;
    for (int loc = 0; loc < length; loc++) {
        stdev += pow(ListItems[loc] - getMean(), 2);
    }
    stdev = sqrt((1/(double)(length))*stdev);
    return stdev;
}

 AUList AUList::DuplicateNV(int N) { //Duplicates the first N values of the list.
    AUList DupList;
    for (int loc = 0; loc < N; loc++) {
         DupList.PutItem(ListItems[loc]);
    }
    return DupList;
}