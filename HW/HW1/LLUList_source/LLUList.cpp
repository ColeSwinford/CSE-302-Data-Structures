...
double LLUList::getMean() { //Gets arithmetic mean.
    double mean = 0;
    LNode* LenPos = ListStart;
    int length = 0;
    while (LenPos != NULL) {
    ...
    for (int loc = 0; loc < length; loc++) {
        mean += (double)GetNextItem();
    }
    mean = mean / length;
    return mean;
}

double LLUList::getStdev() { //Gets the Population Standard Deviation for the list.
    double stdev = 0;
    float mean = getMean();
    ...
    for (int loc = 0; loc < length; loc++) {
        stdev += pow(GetNextItem() - mean, 2);
    }
    stdev = sqrt((1 / (double)length) * stdev);
    return stdev;
}

LLUList LLUList::DuplicateNV(int N) { //Duplicates the first N values of the list.
    ResetList();
    LLUList DupList;
